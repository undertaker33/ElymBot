...
class QqOneBotRuntimeGuardrailTest {
    private val projectRoot: Path = generateSequence(Path.of("").toAbsolutePath()) { current ->
        ...
        Files.exists(candidate.resolve("app/src/main/java/com/elymbot/android"))
    } ?: error("Could not locate project root from ${Path.of("").toAbsolutePath()}")
    private val qqRuntimeRoot: Path =
        projectRoot.resolve("feature/qq/runtime/src/main/java/com/elymbot/android/feature/qq/runtime")
    private val testRoot: Path = projectRoot.resolve("app/src/test/java/com/elymbot/android")
