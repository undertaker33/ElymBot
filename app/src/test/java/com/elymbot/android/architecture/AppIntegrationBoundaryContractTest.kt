...
class AppIntegrationBoundaryContractTest {
    ...
    fun app_integration_must_not_own_core_backup_contract_packages() {
        val violations = kotlinFilesUnder(appIntegrationRoot)
            .filter { file ->
                file.readText().contains("package com.elymbot.android.core.db.backup")
            }
            .map(::relativePath)
    ...
    fun backup_data_port_contracts_must_be_owned_by_app_backup_not_app_integration() {
        val appBackupContract = settingsApiMainRoot
            .resolve("com/elymbot/android/feature/settings/api/backup/AppBackupDataPort.kt")
        val conversationBackupContract = settingsApiMainRoot
            .resolve("com/elymbot/android/feature/settings/api/backup/ConversationBackupDataPort.kt")
        val staleCorePackageContracts = kotlinFilesUnder(
            settingsApiMainRoot.resolve("com/elymbot/android/core/db/backup"),
        ).map(::relativePath)
        val appImportViolations = kotlinFilesUnder(appMainRoot)
            .filter { file -> file.readText().contains("com.elymbot.android.app.integration.backup") }
            .map(::relativePath)
        val appIntegrationContractViolations = kotlinFilesUnder(appIntegrationRoot)
            .filter { file ->
                val text = file.readText()
                text.startsWith("package com.elymbot.android.app.integration.backup") &&
                    (text.contains("interface ") || text.contains("data class "))
            }
            .map(::relativePath)
        val appIntegrationBackupImports = kotlinFilesUnder(appIntegrationRoot)
            .filter { file ->
                file.readText().contains("com.elymbot.android.feature.settings.api.backup.AppBackupDataPort")
            }
            .map(::relativePath)
        ...
        assertTrue(
            ...
            appBackupContract.exists() &&
                appBackupContract.readText().contains("package com.elymbot.android.feature.settings.api.backup") &&
        ...
        assertTrue(
            ...
            conversationBackupContract.exists() &&
                conversationBackupContract.readText().contains(
                    "package com.elymbot.android.feature.settings.api.backup",
                ) &&
