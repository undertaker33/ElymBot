...
class StrictHiltOnlyStartupHotspotSourceTest {
    ...
    fun hilt_probe_and_container_installer_must_not_force_static_app_context_initialization() {
        ...
        val installerSource = projectRoot
            .resolve("core/runtime-container/src/main/java/com/elymbot/android/core/runtime/container/ContainerRuntimeInstaller.kt")
            .readText()
    ...
    @Test
    fun static_runtime_storage_hotspots_must_not_use_elymbot_database_get() {
        val downloadManagerSource = projectRoot
            .resolve("download/impl/src/main/java/com/elymbot/android/download/DefaultDownloadManager.kt")
            .readText()
        val localStoreSource = projectRoot
            .resolve("feature/qq/data/src/main/java/com/elymbot/android/feature/qq/data/NapCatLoginLocalStore.kt")
            .readText()

        assertTrue(
            "DownloadManager must not reach Room via ElymBotDatabase.get(...)",
            !downloadManagerSource.contains("ElymBotDatabase.get("),
        )
        assertTrue(
            "NapCatLoginLocalStore must not reach Room via ElymBotDatabase.get(...)",
            !localStoreSource.contains("ElymBotDatabase.get("),
        )
    }
