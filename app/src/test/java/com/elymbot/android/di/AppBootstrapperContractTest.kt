...
import com.elymbot.android.di.startup.observePluginRuntimeRecords
...
class AppBootstrapperContractTest {
    ...
    fun bootstrap_delegates_to_runner_without_hand_assembled_startup_calls() {
        ...
        assertTrue(
            ...
            source.contains("private val appStartupRunner: com.elymbot.android.di.startup.AppStartupRunner") ||
    ...
    fun plugin_repository_updates_trigger_repeated_loader_syncs() = runTest {
        ...
        try {
            advanceUntilIdle()
            records.value = listOf(pluginV2Record("com.elymbot.samples.container_sync_a"))
            advanceUntilIdle()
            records.value = listOf(
                pluginV2Record("com.elymbot.samples.container_sync_a"),
                pluginV2Record("com.elymbot.samples.container_sync_b"),
            )
            advanceUntilIdle()
            ...
            assertEquals(
                listOf("com.elymbot.samples.container_sync_a", "com.elymbot.samples.container_sync_b"),
    ...
    fun superseded_collect_latest_sync_is_not_reported_as_failure() = runTest {
        ...
        try {
            advanceUntilIdle()
            records.value = listOf(pluginV2Record("com.elymbot.samples.container_cancel_a"))
            advanceUntilIdle()
            records.value = listOf(
                pluginV2Record("com.elymbot.samples.container_cancel_a"),
                pluginV2Record("com.elymbot.samples.container_cancel_b"),
            )
            advanceUntilIdle()
            ...
            assertEquals(
                listOf("com.elymbot.samples.container_cancel_a", "com.elymbot.samples.container_cancel_b"),
    ...
    private fun appBootstrapperSource(): String {
        val sourceFile = listOf(
            File("src/main/java/com/elymbot/android/di/AppBootstrapper.kt"),
            File("app/src/main/java/com/elymbot/android/di/AppBootstrapper.kt"),
        ).first { it.exists() }
