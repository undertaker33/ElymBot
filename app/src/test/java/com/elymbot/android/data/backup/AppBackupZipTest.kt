...
class AppBackupZipTest {
    ...
    fun `write zip stores manifest and files`() {
        ...
        writeAppBackupZip(
            targetZip = zipFile,
            manifestJson = JSONObject().put("schema", "elymbot-android-full-backup-v1"),
            files = listOf(
        ...
        assertEquals("elymbot-android-full-backup-v1", extracted.manifestJson.getString("schema"))
    ...
    fun `read backup payload file understands zip manifests`() {
        ...
        writeAppBackupZip(
            targetZip = zipFile,
            manifestJson = JSONObject().put("schema", "elymbot-android-full-backup-v1").put("createdAt", 7L),
            files = emptyList(),
        )

        val payload = readAppBackupPayloadFile(zipFile, File(tempDir, "unzipped"))

        assertEquals("elymbot-android-full-backup-v1", payload.manifestJson.getString("schema"))
