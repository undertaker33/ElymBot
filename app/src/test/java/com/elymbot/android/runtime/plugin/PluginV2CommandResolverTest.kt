...
class PluginV2CommandResolverTest {
    ...
    fun resolve_prefers_longest_leaf_match_and_preserves_alias_matches() {
        ...
        val canonicalMatch = resolver.resolve("elymbot plugin list all")!!
        assertEquals(listOf("elymbot", "plugin", "list"), canonicalMatch.commandPath)
        assertEquals("elymbot plugin list", canonicalMatch.matchedAlias)
        assertEquals("all", canonicalMatch.remainingText)
        assertEquals(listOf("elymbot", "plugin", "list"), canonicalMatch.bucket.commandPath)
        assertEquals(1, canonicalMatch.bucket.handlers.size)

        val aliasMatch = resolver.resolve("elymbot plugin ls all")!!
        assertEquals(listOf("elymbot", "plugin", "list"), aliasMatch.commandPath)
        assertEquals("elymbot plugin ls", aliasMatch.matchedAlias)
        assertEquals("all", aliasMatch.remainingText)
        assertEquals(listOf("elymbot", "plugin", "list"), aliasMatch.bucket.commandPath)
        assertEquals(listOf(listOf("elymbot", "plugin", "ls")), aliasMatch.bucket.aliasPaths)
    }
    ...
    fun resolve_returns_null_when_input_only_matches_group_prefix() {
        ...
        assertNull(resolver.resolve("elymbot plugin anything"))
    }
    ...
    fun resolve_merges_shared_canonical_paths_from_multiple_plugins_into_one_bucket() {
        ...
        val result = resolver.resolve("elymbot plugin list")!!

        assertEquals(listOf("elymbot", "plugin", "list"), result.commandPath)
