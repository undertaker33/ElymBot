...
class PluginV2LifecycleManagerTest {
    @Test
    fun elymbot_loaded_is_one_shot_for_active_plugins() = runTest {
        ...
        val fixtureA = activeFixture(
            ...
            handlers = listOf(
                lifecycleHandler(
                    ...
                    registrationKey = "elymbot.alpha",
                    handle = recordingHandle("alpha-elymbot", calls),
                ),
            ),
        )
        val fixtureB = activeFixture(
            ...
            handlers = listOf(
                lifecycleHandler(
                    hook = PluginLifecycleHookSurface.OnElymBotLoaded,
                    registrationKey = "elymbot.beta",
                    handle = recordingHandle("beta-elymbot", calls),
                ),
            ),
        )
        ...
        manager.onElymBotLoaded()
        manager.onElymBotLoaded()

        assertEquals(listOf("alpha-elymbot", "beta-elymbot"), calls)
