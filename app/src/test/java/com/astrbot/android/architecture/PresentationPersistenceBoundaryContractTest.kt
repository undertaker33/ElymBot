...
class PresentationPersistenceBoundaryContractTest {
    ...
    private val allowlistFile: Path =
        projectRoot.resolve("app/src/test/resources/architecture/presentation-persistence-allowlist.txt")
    private val allowlist: ImportAllowlist = loadAllowlist()

    @Test
    fun allowlist_file_must_exist_and_be_well_formed() {
        assertTrue(
            "Presentation persistence allowlist file must exist: $allowlistFile",
        ...
        assertTrue(
            "Presentation persistence allowlist contains malformed entries: $malformedEntries",
    ...
    fun allowlist_entries_must_point_to_existing_production_files() {
        ...
        assertTrue(
            "Presentation persistence allowlist points to missing production files: $missing",
            missing.isEmpty(),
        )
    }

    @Test
    fun feature_presentation_and_root_ui_must_not_import_database_layer_directly() {
        ...
        assertTrue(
            "feature/*/presentation/** and ui/** must not import com.astrbot.android.data.db.* directly. " +
                "Add only precise temporary allowlist entries for existing debt: $violations",
            violations.isEmpty(),
        )
    }

    private fun presentationFiles(): List<Path> {
        val roots = buildList {
            ...
            if (featureRoot.exists()) {
                Files.list(featureRoot).use { stream ->
                    stream
                        ...
                        .map { path -> path.resolve("presentation") }
                        .filter { path -> path.exists() }
                        .forEach(::add)
                }
            }
            mainRoot.resolve("ui").takeIf { it.exists() }?.let(::add)
        }
        ...
        val entries = allowlistFile.readLines()
            ...
            .mapIndexed { index, line ->
                ...
                require(parts.size == 7) {
                    "Invalid presentation persistence allowlist entry at line ${index + 1}: $line"
                }
