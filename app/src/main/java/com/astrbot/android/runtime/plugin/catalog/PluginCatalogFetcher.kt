...
class UrlConnectionPluginCatalogFetcher : PluginCatalogFetcher {
    override suspend fun fetch(catalogUrl: String): String {
        RuntimeLogRepository.append("Plugin market fetch start: url=$catalogUrl")
        ...
        return try {
            ...
            RuntimeLogRepository.append("Plugin market fetch failed: url=$catalogUrl error=${error.toRuntimeLogSummary()}")
            throw error
        } finally {
