...
object CronJobActiveCapabilityTaskRepository : ActiveCapabilityTaskRepository {
    override suspend fun create(job: CronJob): CronJob = CronJobRepository.create(job)
    override suspend fun delete(jobId: String) = CronJobRepository.delete(jobId)
    override suspend fun getByJobId(jobId: String): CronJob? = CronJobRepository.getByJobId(jobId)
    override suspend fun listAll(): List<CronJob> = CronJobRepository.listAll()
    override suspend fun latestExecutionRecord(jobId: String): CronJobExecutionRecord? {
        return CronJobRepository.latestExecutionRecord(jobId)
    }
}
...
class ActiveCapabilityRuntimeFacade(
    ...
    private fun inferTomorrowSchedule(
        ...
        val mentionsTomorrow = listOf("明天", "明早", "明晚", "tomorrow").any { token ->
            text.contains(token, ignoreCase = true)
        }
        if (!mentionsTomorrow) return null
        val targetTime = when {
            text.contains("明晚") || text.contains("晚上") || text.contains("今晚") || text.contains("evening", ignoreCase = true) -> LocalTime.of(20, 0)
            text.contains("中午") || text.contains("noon", ignoreCase = true) -> LocalTime.of(12, 0)
            text.contains("下午") || text.contains("afternoon", ignoreCase = true) -> LocalTime.of(15, 0)
            text.contains("早上") || text.contains("上午") || text.contains("明早") || text.contains("morning", ignoreCase = true) -> LocalTime.of(9, 0)
...
private val relativeDelayMatchers: List<(String, ZonedDateTime) -> ZonedDateTime?> = listOf(
    { text, now ->
        Regex("([零一二两兩三四五六七八九十百\\d]+)\\s*(分钟|分鐘|分鍾|min|mins|minute|minutes)后", RegexOption.IGNORE_CASE)
            .find(text)
            ?.groupValues
            ?.getOrNull(1)
            ?.toFlexibleLong()
    ...
    { text, now ->
        Regex("([零一二两兩三四五六七八九十百\\d]+)\\s*(小时|小時|hr|hrs|hour|hours)后", RegexOption.IGNORE_CASE)
            .find(text)
            ?.groupValues
            ?.getOrNull(1)
            ?.toFlexibleLong()
    ...
    { text, now ->
        Regex("([零一二两兩三四五六七八九十百\\d]+)\\s*(天|day|days)后", RegexOption.IGNORE_CASE)
            .find(text)
            ?.groupValues
            ?.getOrNull(1)
            ?.toFlexibleLong()
