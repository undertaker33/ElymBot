...
class ElymBotHttpClientTest {
    ...
    fun `execute delegates to runtime transport and preserves response payload contract`() {
        ...
        val client = OkHttpElymBotHttpClient(
            transport = transport,
    ...
    fun `execute bytes delegates to runtime transport`() {
        ...
        val client = OkHttpElymBotHttpClient(transport = transport)
    ...
    fun `execute bytes on runtime http failure includes status and body`() {
        ...
        assertTrue(error is ElymBotHttpException)
    ...
    fun `sanitize error body redacts api keys`() {
        val client = OkHttpElymBotHttpClient(transport = RecordingRuntimeTransport())
