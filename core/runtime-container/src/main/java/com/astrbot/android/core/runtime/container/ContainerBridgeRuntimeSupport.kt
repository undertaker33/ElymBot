...
object ContainerBridgeRuntimeSupport {
    ...
    fun buildProgressNotificationText(runtimeState: ContainerRuntimeState): String {
        return when (runtimeState.statusType) {
            ContainerRuntimeStatus.RUNNING -> "NapCat running"
            ContainerRuntimeStatus.ERROR -> "NapCat start failed"
            ContainerRuntimeStatus.STOPPED -> "NapCat stopped"
            else -> {
...
data class RuntimeProgressSnapshot(
    val label: String = "",
    val percent: Int = 0,
