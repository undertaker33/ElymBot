...
class QuickJsExternalPluginScriptExecutor(
    ...
    private fun buildQuickJsModuleSource(
        ...
        return buildString {
            ...
            appendLine()
            appendLine("const __elymbotContext = JSON.parse($quotedContextJson);")
            appendLine("const __elymbotEntryName = $quotedEntrySymbol;")
            appendLine("globalThis.$QUICKJS_RESULT_PROPERTY = '';")
            appendLine("const __elymbotEntry = typeof globalThis[__elymbotEntryName] === 'function' ? globalThis[__elymbotEntryName] : (typeof handleEvent === 'function' && __elymbotEntryName === 'handleEvent' ? handleEvent : undefined);")
            appendLine("if (typeof __elymbotEntry !== 'function') {")
            appendLine("  throw new Error(`Missing entry symbol: ${'$'}{__elymbotEntryName}`);")
            appendLine("}")
            appendLine("const __elymbotResult = __elymbotEntry(__elymbotContext);")
            appendLine("if (__elymbotResult && typeof __elymbotResult.then === 'function') {")
            appendLine("  throw new Error('Async QuickJS entry is not supported in v1.');")
            appendLine("}")
            appendLine("globalThis.$QUICKJS_RESULT_PROPERTY = JSON.stringify(__elymbotResult ?? { resultType: 'noop', reason: 'External plugin returned no result.' });")
            appendLine("globalThis.$QUICKJS_RESULT_PROPERTY;")
        }
    }

    companion object {
        internal const val QUICKJS_RESULT_PROPERTY = "__elymbotSerializedResult"
        internal const val QUICKJS_BOOTSTRAP_CALLABLE_PROPERTY = "__elymbotBootstrapCallable"
        internal const val QUICKJS_BOOTSTRAP_HOST_API_PROPERTY = "__elymbotBootstrapHostApi"
...
private class QuickJsExternalPluginBootstrapSession(
                ...
                "messages" to request.messages.map { message ->
                    linkedMapOf(
                        ...
                        "parts" to message.parts.map { part ->
                            when (part) {
                                is PluginProviderMessageTextPart -> linkedMapOf(
                                    "partType" to "text",
                                    "text" to part.text,
                                )

                                is PluginProviderMessageMediaRefPart -> linkedMapOf(
    ...
    private fun parsePluginMessageEventResultAttachments(
        value: Any?,
    ): List<PluginMessageEventResultAttachment> {
    ...
    private fun bootstrapExecutionSourcePath(): String {
        return File(pluginRootDirectory, "__elymbot_bootstrap_exec__.js")
            .absolutePath
            .replace('\\', '/')
    }

    private fun bootstrapPollSourcePath(): String {
        return File(pluginRootDirectory, "__elymbot_bootstrap_poll__.js")
            .absolutePath
            .replace('\\', '/')
    }
    ...
    private fun resolveBootstrapCallable(
        ...
        val bootstrapSpecifier = "./" + pluginRootPath.relativize(bootstrapPath)
            .toString()
            .replace('\\', '/')
        val loaderModulePath = File(pluginRootDirectory, "__elymbot_runtime_loader__.mjs")
            .absolutePath
            .replace('\\', '/')
        val loaderSource = buildString {
            appendLine("import * as __elymbotBootstrapModule from ${JSONObject.quote(bootstrapSpecifier)};")
            appendLine("const __elymbotBootstrapCandidates = [];")
            appendLine("if (typeof __elymbotBootstrapModule.default === 'function') {")
            appendLine("  __elymbotBootstrapCandidates.push(__elymbotBootstrapModule.default);")
            appendLine("}")
            appendLine("if (typeof __elymbotBootstrapModule.bootstrap === 'function') {")
            appendLine("  __elymbotBootstrapCandidates.push(__elymbotBootstrapModule.bootstrap);")
            appendLine("}")
            appendLine("if (__elymbotBootstrapCandidates.length === 0) {")
            appendLine("  throw new Error('Missing bootstrap callable. Expected a default export or named bootstrap().');")
            appendLine("}")
            appendLine("if (__elymbotBootstrapCandidates.length > 1) {")
            appendLine("  throw new Error('Bootstrap module must resolve to a single callable.');")
            appendLine("}")
            appendLine("globalThis.${QuickJsExternalPluginScriptExecutor.QUICKJS_BOOTSTRAP_CALLABLE_PROPERTY} = __elymbotBootstrapCandidates[0];")
...
private fun buildQuickJsBootstrapExecutionSource(): String {
    return buildString {
        ...
        appendLine("  try {")
        appendLine(
            "    const __elymbotHasBootstrapHostApi = Object.prototype.hasOwnProperty.call(globalThis, ${JSONObject.quote(QuickJsExternalPluginScriptExecutor.QUICKJS_BOOTSTRAP_HOST_API_PROPERTY)});",
        )
        appendLine("    if (__elymbotHasBootstrapHostApi) {")
        appendLine(
