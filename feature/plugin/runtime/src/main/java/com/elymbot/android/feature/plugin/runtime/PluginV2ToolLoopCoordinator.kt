    ...
    private fun buildToolResultMessage(
        ...
        val parts = buildList<PluginProviderMessagePartDto> {
            result.text?.takeIf { it.isNotEmpty() }?.let { text ->
                add(PluginProviderMessageTextPart(text))
            }
            result.structuredContent?.takeIf { it.isNotEmpty() }?.let { structured ->
                add(PluginProviderMessageTextPart(PluginExecutionProtocolJson.canonicalJson(structured)))
            }
            if (isEmpty()) {
                add(PluginProviderMessageTextPart(""))
            }
        }
