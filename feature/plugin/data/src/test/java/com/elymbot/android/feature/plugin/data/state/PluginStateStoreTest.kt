package com.elymbot.android.feature.plugin.data.state

import com.elymbot.android.data.db.PluginStateEntryDao
import com.elymbot.android.data.db.PluginStateEntryEntity
...
class PluginStateStoreTest {
    ...
    fun plugin_state_entry_dao_like_queries_must_use_escape_clause() {
        ...
        val source = projectRoot
            .resolve("core/db/src/main/java/com/elymbot/android/data/db/plugin/PluginStateEntryDao.kt")
            .readText()
    ...
    private fun detectProjectRoot(): Path {
        return generateSequence(Path.of("").toAbsolutePath()) { path -> path.parent }
            .firstOrNull { path -> path.resolve("settings.gradle.kts").exists() }
            ?: error("Unable to resolve project root from ${Path.of("").toAbsolutePath()}")
    }
}
