...
open class FeatureQqPlatformConfigPortAdapter private constructor(
    ...
    constructor(
        botsSnapshot: () -> List<BotProfile>,
        selectedBotIdSnapshot: () -> String,
        configReader: (String) -> ConfigProfile,
    ) : this(
        resolveQqBotReader = { selfId ->
            resolveQqBot(
                bots = botsSnapshot(),
                selectedBotId = selectedBotIdSnapshot(),
                selfId = selfId,
            )
        },
        botReader = { botId -> botsSnapshot().firstOrNull { it.id == botId } },
