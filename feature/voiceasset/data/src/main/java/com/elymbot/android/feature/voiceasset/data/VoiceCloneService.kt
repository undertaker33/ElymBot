...
class VoiceCloneService @Inject constructor(
    ...
    override fun cloneVoice(
        ...
        return when (provider.providerTypeName) {
            "BAILIAN_TTS" -> cloneQwenVoice(provider, asset, displayName)
            "MINIMAX_TTS" -> cloneMiniMaxVoice(provider, asset, displayName)
            else -> throw IllegalStateException("Voice cloning is not supported for ${provider.providerTypeName}.")
        }
    }
    ...
    private fun uploadMiniMaxFile(
        ...
        val boundary = "ElymBotBoundary${System.currentTimeMillis()}"
    ...
    private fun mergeLocalClipsToWav(asset: TtsVoiceReferenceAsset): ReferenceAudioData? {
        ...
        val gapFrames = ((first.sampleRate * clipGapMs) / 1000.0).toInt().coerceAtLeast(0)
